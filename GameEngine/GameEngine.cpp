...
static bool setupConsolse(HINSTANCE hInstance) {
    if (!GameEngine::Windows::Utils::createNewConsole(CONSOLE_BUFFER_SIZE)) {
        WCHAR errorTitle[MAX_LOADSTRING];
        WCHAR errorMessage[MAX_LOADSTRING];
        LoadStringW(hInstance, IDS_GENERIC_ERROR_TITLE, errorTitle, MAX_LOADSTRING);
        LoadStringW(hInstance, IDS_ERROR_OPENING_CONSOLE, errorMessage, MAX_LOADSTRING);
